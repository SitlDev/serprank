...
export function SearchVolumeChart({ data, title = 'Search Volume Trend', height = '300px' }: SearchVolumeChartProps) {
  ...
  const points = data.map((d, i) => {
    ...
    const y = padding + chartHeight - ((d.volume - minVolume) / range) * chartHeight
    return { x, y, ...d }
  })
    ...
    <div className="bg-white rounded-lg shadow p-6" style={{ height }}>
      ...
      <div className="overflow-x-auto">
        <svg width={width} height={chartHeight + padding * 2} className="mx-auto">
          ...
          {[0, 0.25, 0.5, 0.75, 1].map((ratio, i) => (
            ...
            >
              {Math.round(minVolume + ratio * range).toLocaleString()}
            </text>
          ))}
      ...
      <div className="mt-4 grid grid-cols-3 gap-4 text-sm">
        <div>
          ...
          <span className="font-semibold text-gray-900 ml-2">{data[data.length - 1].volume.toLocaleString()}</span>
        </div>
        <div>
        ...
        <div>
          ...
          <span className="font-semibold text-gray-900 ml-2">{maxVolume.toLocaleString()}</span>
        </div>
      </div>
    </div>
  )
