...
export function TrafficChart({ data, title = 'Estimated Traffic', height = '300px' }: TrafficChartProps) {
  ...
  // Calculate max traffic for scaling
  const maxTraffic = Math.max(...data.map(d => d.traffic))
  const minTraffic = Math.min(...data.map(d => d.traffic))
  ...
  const points = data.map((d, i) => {
    ...
    const y = padding + chartHeight - ((d.traffic - minTraffic) / range) * chartHeight
    return { x, y, ...d }
  })
  ...
  // Determine trend
  const trend = data[data.length - 1].traffic > data[0].traffic ? 'up' : 'down'
  const change = Math.round(((data[data.length - 1].traffic - data[0].traffic) / data[0].traffic) * 100)
    ...
    <div className="bg-white rounded-lg shadow p-6" style={{ height }}>
      ...
      <div className="overflow-x-auto">
        <svg width={width} height={chartHeight + padding * 2} className="mx-auto">
          ...
          {[0, 0.25, 0.5, 0.75, 1].map((ratio, i) => (
            ...
            >
              {Math.round(minTraffic + ratio * range).toLocaleString()}
            </text>
          ))}
      ...
      <div className="mt-4 grid grid-cols-3 gap-4 text-sm">
        <div>
          ...
          <span className="font-semibold text-gray-900 ml-2">{Math.round(data[data.length - 1].traffic).toLocaleString()}</span>
        </div>
        <div>
          <span className="text-gray-600">Average:</span>
          <span className="font-semibold text-gray-900 ml-2">
            {Math.round(data.reduce((sum, d) => sum + d.traffic, 0) / data.length).toLocaleString()}
          </span>
        </div>
        <div>
